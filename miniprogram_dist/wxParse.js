...
var _showdown = __webpack_require__(14);
...
Component({
      ...
      observer: function observer(val) {
        ...
        if (val) {
          ...
          if (language === 'markdown' || language === 'md') {
            ...
            var parseNodes = converter.makeHtml(val);
  ...
  detached: function detached() {
    // 组件销毁，清除绑定实例
    _util.bindInstance.clear();
  },


  methods: {
    _parseHtml: function _parseHtml(html, bindName) {
      var _bindData;

      bindName = 'wxParseData';

      //存放html节点转化后的json数据
      var transData = _html2json2.default.html2json(html, bindName);

      transData.view = {};
      transData.view.imagePadding = 0;
      this.setData({
        nodesData: transData.nodes,
        bindData: (_bindData = {}, _bindData[bindName] = transData, _bindData)
      });
      _util.bindInstance.set(bindName, transData);
      console.log(this.data);
    },
    _parseNodes: function _parseNodes(nodes) {
    ...
    wxParseImgLoad: function wxParseImgLoad(e) {
      ...
      if (typeof tagFrom !== 'undefined' && tagFrom.length > 0) {
        ...
        }, _setData['nodesData[' + index + '].loaded'] = true, _setData));
      }
    },
    ...
    wxParseImgTap: function wxParseImgTap(e) {
      ...
      var _bindInstance$set = _util.bindInstance.set(bindName),
          imageUrls = _bindInstance$set.imageUrls;
     ...
     */
    _wxAutoImageCal: function _wxAutoImageCal(originalWidth, originalHeight) {
      var autoWidth = 0,
          autoHeight = 0;
      var results = {};

      var _getSystemInfo = (0, _util.getSystemInfo)(),
          windowWidth = _getSystemInfo[0],
          windowHeight = _getSystemInfo[1];
    ...
    wxParseTagATap: function wxParseTagATap(e) {
      var _e$currentTarget$data = e.currentTarget.dataset.src,
          src = _e$currentTarget$data === undefined ? '' : _e$currentTarget$data;
      ...
      // 判断是否内部链接跳转
      var isInnerPage = src.indexOf('http') === -1;
      if (isInnerPage) {
        wx.navigateTo({
          url: src
        });
...
var __emojisBaseSrc = '';
var __emojis = {};
var wxDiscode = __webpack_require__(12);
var HTMLParser = __webpack_require__(13);
...
var bindInstance = function () {
  var instance = null;

  return {
    set: function set(bindName) {
      var data = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;

      if (!instance) {
        var _instance;

        instance = (_instance = {}, _instance[bindName] = data, _instance);
      }
